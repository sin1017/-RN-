...
function body(props) {
  ...
  function randomItem(value) {
    ...
    if ([2, 4, 5, 7, 10, 12, 13, 15].includes(value.index + 1)) {
      boardStyle = 'gridBoard'
    }
    if (
      value.item.some((value) => value === null)
    ) {
      boardStyle = [2, 4, 5, 7, 10, 12, 13, 15].includes(value.index + 1) ? 'greenGridBoard' : 'greenBoard';
    }

    return <View>
      <TouchableOpacity
        onPress={() => props.getAddChessPosition(value.index)}
      ...
      >
        ...
        >
          {
            value.item.image && <Image source={require('../assets/chess/red-chess.png')} />
          }
        </View>
      </TouchableOpacity>
    </View>
  ...
  return (
    <View>
      ...
      <View style={style.main}>
        ...
        <FlatList data={props.boardList} renderItem={randomItem} keyExtractor={(item, index) => { item, index }}>
          {
            // <View>
            //   <TouchableOpacity
            //     onPress={() => props.getAddChessPosition(index, item)}
